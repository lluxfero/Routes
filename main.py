...
# импортируем функции для генерации банкоматов и симуляции
from module_simulation import generate_atms, simulate_service_needs  
from module_routes import plan_routes  # импортируем функцию для планирования маршрутов

import matplotlib.pyplot as plt

# визуализация маршрутов инкассаторов
def plot_routes(atms, planned_routes):  # функция для построения маршрутов
    for day, route_info in planned_routes.items():  # итерируем по каждому дню и информации о маршруте
        ...
        # отфильтровываем обслуженные банкоматы
        ...
        # наносим точки обслуженных банкоматов на график зеленым цветом
        ...
        for group_id, route in route_info['routes'].items():  # итерируем по каждой группе маршрутов
            ...
            # наносим линии маршрутов на график
        ...
        if route_info['unassigned']:  # проверяем, есть ли не назначенные банкоматы
            # собираем X-координаты не назначенных банкоматов
            unassigned_x = [atm.x for atm in route_info['unassigned']]  
             # собираем Y-координаты не назначенных банкоматов
            unassigned_y = [atm.y for atm in route_info['unassigned']] 
            # наносим точки не назначенных банкоматов на график красным цветом
...
# сохраняет информацию в текстовый файл
...
# получаем банкоматы, нуждающиеся в обслуживании по дням
atms_needing_service = simulate_service_needs(atms, NUM_DAYS)  
